...
from data.data_utils import rep_files_list, folders_list, files_list, \
    exist_test
import config_path
...
def test_preprocessing(file_types):
    # Teste le nombre d'images traitées
    prep_df = pd.read_csv(config_path.DB_IMG, sep=',', header=0)
    assert prep_df.shape[0] == 2084  # 1944 + 140

    # Teste le nombre d'images pré-processées
    assert prep_df['preprocessing'].sum() == 1789  # 295 Dhashs identiques
...
@pytest.mark.parametrize("variable, expected", [('mdl_integ', 1616),
                                                ('train_sel', 1280),
                                                ('test_sel', 336),
                                                ('wl_mdl_integ', 173)])
def test_db_coherence(variable, expected):
    db = pd.read_csv(config_path.DB_IMG, sep=',', header=0)
